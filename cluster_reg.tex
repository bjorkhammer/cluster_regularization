...
Searching the potential energy surface for the minimum energy structure 


Evolutionary algorithms are frequently use in search for stable atomic structures and potential energy surfaces.


\begin{figure}[h]
    \centering
    \includegraphics[width=1.0\columnwidth]{fig1-intro.pdf}
    \caption{Example of the structures we are working with. Top-left the slab used, top-right a initial structure, bottom-left a structures with some minimisation, bottom-right the global mimimum.}
    \label{figintro}
\end{figure}
...
In figure \ref{figfeature} the feature vector is displayed for 1019 structures of Ti$_{13}$O$_{26}$ with the first component $f_8(i)$ along the x-axis, the second $f_{22}(i)$ along the y-axis and the color is given by the third component $T(i)$. 


\begin{figure}[h]
    \centering
    \includegraphics[width=1.0\columnwidth]{gmascatter.pdf}
    \caption{Visualization of global minimum for Ti$_{21}$O$_{42}$ structures in feature space}   
    \label{figglobal}
\end{figure}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1.0\columnwidth]{fig2-scatterplot.pdf}
...
\begin{equation}
\nabla_{\vec r_i} \text{TCD}(S) = \sum_{i \in S} {\vec f(i) - \vec c_i \over 2 d(\vec f(i), \vec c_i)} \sum_{j \in nl(i)} e^{-d_{ij}/1\text{\AA}} {\vec r_i-\vec r_j \over d_{ij}}
 \label{eq4}
\end{equation}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1.0\columnwidth]{fig5-success.pdf}
