...
occationally to a benchmark EA search, where it is not used at all. We
also test the method in a basin hopping (BH) framework, in which some
perturbing update step alternates a local energy relaxation step, see
Ref.\ \cite{} for details.
...
\begin{equation}
D_S = \sum_{i \in S} \left|\vec f_i- \vec c_{k(i)}\right| \label{eq4}
\end{equation}
...
structure present in the disordered structures.
...
$E_S^\mathrm{DFTB}$, is revealed when a
clustering of the atomic feature vectors present in
Fig.\ \ref{fig_feature} is performed. The correlation is illustrated in
...
raises above that of the global minimum energy structure.

\subsubsection{Cluster regularization method}
...
in Fig.\ \ref{fig_corr} is suggestive that when moved according to (minus) the cluster distance gradient, a given structure may leave a meta-stable funnel
region of the energy landscape and move into a more favorable
funnel in the energy landscape. This is indeed observed to happen and will be illustrated
below when the cluster regularization is used in two different ways,
either as the perturbation step in a basin hopping (BH) setup or as a
...
atomic features to the latest structure in the search.  Figure
...
steps (blue color) lead to pregressively more favorable
structures. Figure \ref{fig_min}a and b show the evolution of the
...
local energy minima, thereby gaining about 0.5 eV over the course of
...
develops. We observe (not shown) that many BH runs end up in a dynamic
equilibrium in which the structure jumps back and forth between the
same two local minima, one on the cluster distance landscape and one
on the energy landscape. However, occationally, the BH runs succeed in
...
serve in a manner simular to that in a basin hopping setup, but owing
to the stochastic elementrs in the EA, the risk of being trapped in a
...
\begin{enumerate}
\item Draw randomly $N_{par}$ parents from the population.
...
\item Minimize the cluster distance of the child.
\item Return child.
\end{enumerate}
...
The optimium values of the free parameters in the new mutation were
determined with bayesian hyper-parameter optimization (BHO.) search
runs on the three-layer system. We found $N_c=5$ clusters, $N_{par}=3$
...
optimum ratio of 70\% cluster regularization, 28\% cut\&splice and 2\%
rattle mutation when cluster regularization was included which
compares to an BHO. based result of 59\% cut\&splice, 0\% permutation,
41\% rattle mutation in the absence of the cluster regularization. The
latter parameters were used for benchmark runs. In both cases, the
BHO. search method included the possibility of performing permutation
mutations, but opted not to do so. In order to gauge the efficiency of
the cluster regularization method on its own we further did EA runs,
using that method exclusively (i.e.\ having 100\% cluster
regulatization and 0\% cut\&splice and 0\% rattle mutation).
...
The three EA modes -- benchmark, all-cluster regularization, and
combined -- were tested for two-, three-, and four-layer TiO$_2$
systems. The results are presented in Fig.\ \ref{fig_success} that
...
method (orange curve) starts finding the global minium energy
structure sooner than the (yellow curve). For the two-layer system,
the all-cluster regularization method levels off at around 60-70\%
with an overall smaller success rate than obtained in the benchmark
...
number of structure (essentially most of the population members).
...
combined method still exhibit much sooner finding of the global energy
minimum structure than the benchmark runs, and no signs are seen of
stagnation.
...
\bibitem{Chen2017} {Understanding atomic chemistry with machine learning.} Chen, X., Jørgenson, M. S., Hammer, B., \& Li, J. (2017) Unpublished
