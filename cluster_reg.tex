...
surface with (1$\times$4) periodicity with lattice parameters $a=3.94$ {\AA} and $c=9.47$ {\AA}. A mesh of 2$\times$1 \textbf{k}-points is used to sample the Brillouin zone.
...
\begin{figure}[tb]
    ...
    \caption{Side views of computational cells with Ti$_{n}$O$_{2n}$ structures.
      ...
      a) The static template of four TiO$_2$ units.
...
also test the method in a basin hopping (BH) framework,\cite{wales1997} in which it serves as a
perturbing update step that alternates with an energy relaxation step.
...
minimum energy structure indicates that despite its simplicity the
...
the basis for Fig.\ \protect\ref{fig_feature}. The $N_c=5$ cluster centers used in Eq.\ (4) were:
$( 22,0.89, 0.19)$,
$( 22,0.53, 0.14)$,
$( 8 ,0.62, 0.44)$,
$( 8 ,0.46, 0.38)$, and
...
\begin{figure}[tb]
    ...
    \caption{a) Structure optimization via alternation between minimization in real space and feature space. b+c) Situations where the structure is stuck in one or more real space local minima due to a dynamic equilibrium between relaxations in real space and feature space.}
    \label{fig_schem}
\end{figure}
...
in Fig.\ \ref{fig_corr} is suggestive that when moved according to (minus) the cluster distance gradient, Eq.\ (\ref{eq_deriv}), a given structure may leave a meta-stable basin
...
\begin{figure*}[tb]
    ...
    \caption{Evolution of energy (in eV/atom), cluster distance, and structures
      ...
      hopping run. d) Structural illustrations of 10 selected
      states from the same run. The progress in the individual
...
use as a cross-over operation in connection with an evolutionary algorithm (EA). It 
serves in a manner similar to that in a basin hopping setup, but owing
...
\item Minimize the cluster distance for the lowest energy parent.
...
When the method returns the child, the EA will make an energy
...
system (12 static + 15 optimized atoms) to the 3-layer and 4-layer systems (12 static + 27 and
...
\bibitem{pickard2011}  C. J. Pickard and R. J. Needs. J. Phys.: Condens. Matter \textbf{23} (2011) 053201 (23pp) DOI:10.1088/0953-8984/23/5/053201
...
\bibitem{mathias2018}M.\ S.\ J{\o}rgensen, U.\ F.\ Larsen, K.\ W.\ Jacobsen, and B.\ Hammer, J.\ Phys.\ Chem.\ A, \textbf{122}, 1504 (2018).
\bibitem{yamashita2018}T.\ Yamashita, N.\ Sato, H.\ Kino, T.\ Miyake, K.\ Tsuda, and T.\ Oguchi, Phys.\ Rev.\ Mater.\ \textbf{2}, 013803 (2018).
...
\bibitem{Arthur07k-means-plus-plus}D.\ Arthur and S.\ Vassilvitskii, \textit{K-means++: the advantages of careful seeding} in
